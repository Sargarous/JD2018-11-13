...
public class FrontController extends HttpServlet {
    ...
    private void process(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        by.it.kruglenja.Project.java.controller.Action action = by.it.kruglenja.Project.java.controller.Action.definer(req);
        by.it.kruglenja.Project.java.controller.Action next = null;
        try {
            next = action.cmd.execute(req);
        } catch (Exception e) {
            req.setAttribute("message", e.toString());
            getJsp(req, resp, by.it.kruglenja.Project.java.controller.Action.INDEX);
